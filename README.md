...
This project uses the [agent-system](https://github.com/your-org/agent-system) framework for AI-assisted development. Framework files are **not committed to this repo** — they are managed externally and regenerated by `sync.py`.

After cloning, bootstrap the framework:

```bash
# From the agent-system repo:
python sync.py --target /path/to/voxema --render
```

Or, if Voxema is registered in `downstream.projects`:

```bash
python sync.py --all --render
```

Framework updates propagate automatically when you commit changes in agent-system (via post-commit hook).
