...
This document defines the architectural constraints that all agents must respect when working on **Voxema**.
...
The system is built around a deterministic processing pipeline: `capture → transcribe → diarize → summarize → export`
...
Voxema should prefer **deterministic behavior wherever practical**.
